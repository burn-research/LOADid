...
for model_id, cfg in tqdm(MODEL_DEFS.items(), desc='Models', unit='model'):
    ...
    for customer, df_customer in tqdm(df_elec.groupby('EAN_ID'), desc='Customers', unit='customer'):
        ...
        heat_flag = df_customer['HP'].iloc[0] == 1 # 1
        pv_flag = df_customer['PV'].iloc[0] == 1 # 1
...
#%% Model comparison (HP and PV only)

# MLP - HP and PV only
df_mlp = pd.read_pickle('mlp_results.pkl')
df_mlp_hp_pv = df_mlp[(df_mlp['HP'] == 1) & (df_mlp['PV'] == 1) & (df_mlp['EV'] == 0)]

model_order = ['M0', 'M1', 'M2', 'M3', 'M4', 'M5']
params = ['y0', 'alpha', 'Th', 'rho']

# HP and PV only
df_results_hp_pv = df_results[(df_results['HP'] == 1) & (df_results['PV'] == 1) & (df_results['EV'] == 0)]
...
r2_mean = [df_results_hp_pv[df_results_hp_pv['MODEL'] == model]['R2'].mean() for model in model_order] + [df_mlp_hp_pv['R2'].mean()]
r2_std = [df_results_hp_pv[df_results_hp_pv['MODEL'] == model]['R2'].std() for model in model_order] + [df_mlp_hp_pv['R2'].std()]

rmse_mean = [df_results_hp_pv[df_results_hp_pv['MODEL'] == model]['RMSE'].mean() for model in model_order] + [df_mlp_hp_pv['RMSE'].mean()]
rmse_std = [df_results_hp_pv[df_results_hp_pv['MODEL'] == model]['RMSE'].std() for model in model_order] + [df_mlp_hp_pv['RMSE'].std()]

aic_mean = [df_results[df_results['MODEL'] == model]['AIC'].mean() for model in model_order] + [df_mlp_hp_pv['AIC'].mean()]
bic_mean = [df_results[df_results['MODEL'] == model]['BIC'].mean() for model in model_order] + [df_mlp_hp_pv['BIC'].mean()]
...
plt.savefig(os.path.join(BASE_DIR, f'../../Figures/Regression/metrics_elec_model.png'), dpi=300, bbox_inches='tight')
plt.savefig(os.path.join(BASE_DIR, f'../../Figures/Regression/metrics_elec_model.pdf'), bbox_inches='tight')
